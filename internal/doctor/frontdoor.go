...
// door with Linx's own certificate (so the front door passes it through
// untouched), the relay works over TLS on 443 through it, coturn answers on
// UDP 443, the names are in DNS, and (Pangolin) only Pangolin may reach the
// web port. The router's forwards themselves can't be seen from inside.
func FrontDoor(ctx context.Context, env Env, cfg installer.Config) []Result {
	...
	case installer.FrontDoorPangolin:
		entry, _ = netip.ParseAddr(fd.PangolinAddress)
		door = "Pangolin (" + fd.PangolinAddress + ")"
	...
	default:
		rs.warn("Calls from outside your home aren't set up (no front door chosen).",
	...
	for _, h := range installer.PublicHosts {
		...
		got, err := env.TLSLeaf(ctx, addr, name, roots)
		switch {
		case err != nil:
	...
	relayOverTLS(ctx, env, &rs, addr, "turn."+cfg.Domain.Name, roots, door, fix)
	relayUDP(ctx, env, &rs, settings, fd.Kind)
	publicDNS(ctx, env, &rs, cfg)
	if fd.Kind == installer.FrontDoorPangolin {
		frontDoorFirewall(ctx, env, &rs, settings.WebClients)
	}
	return rs
}

func frontDoorFix(kind string) string {
	if kind == installer.FrontDoorPangolin {
		return "Check Pangolin has Linx's settings: " + installer.PangolinStepsFile + " (step 1). Then run linx doctor again."
	}
...
// publicDNS checks the public names resolve, to one public address.
func publicDNS(ctx context.Context, env Env, rs *results, cfg installer.Config) {
...
func frontDoorFirewall(ctx context.Context, env Env, rs *results, want []netip.Addr) {
	...
	if err != nil || !slices.Equal(loaded, wantP) {
		rs.fail("The firewall doesn't limit Linx's web port to Pangolin.", rerunSetup)
		return
	}
	rs.ok("Only Pangolin (" + want[0].String() + ") can reach Linx's web port on this network.")
}
