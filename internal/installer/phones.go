...
func PhonesPlan(ctx context.Context, r Runner, lan LAN, readFile func(string) ([]byte, error)) (Plan, error) {
	var p Plan
	...
	p = append(p,
		fileStep("Write the firewall rules: "+what+"; never allow unencrypted SIP (port 5060)",
			FirewallRules, FirewallRuleset(lan), 0o644, 0o755),
...
// replaces the table atomically.
func FirewallRuleset(lan LAN) []byte {
	elements := ""
	if lan.OK() {
...
`, FirewallUnit, sip, rtp, FirewallTable, elements)
}
