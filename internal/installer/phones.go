...
func PhonesPlan(ctx context.Context, r Runner, lan LAN, web []netip.Addr, readFile func(string) ([]byte, error)) (Plan, error) {
	var p Plan
	...
	p = append(p,
		fileStep("Write the firewall rules: "+what+"; never allow unencrypted SIP (port 5060)",
			FirewallRules, FirewallRuleset(lan, web), 0o644, 0o755),
...
// replaces the table atomically. web are the front door's addresses (Pangolin
// at home): the only ones that may reach the web port and coturn's TLS
// port when they're published on the LAN (docs/WEB.md §3).
func FirewallRuleset(lan LAN, web []netip.Addr) []byte {
	elements := ""
	if lan.OK() {
...
# from the front door (Pangolin), if any.
...
%[8]s	}
	...
	chain prerouting {
		...
		fib daddr type local tcp dport { %[6]d, %[7]d } ip saddr @front_door accept
		fib daddr type local tcp dport { %[6]d, %[7]d } counter drop
	}
}
`, FirewallUnit, sip, rtp, FirewallTable, elements, WebPort, TURNTLSPort, webElements)
}
