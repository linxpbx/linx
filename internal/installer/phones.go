...
const (
	...
	FirewallTable = "linx"
	// TrunkAddressSet holds every enabled IP-authenticated trunk's
	// resolved address (docs/TRUNKS.md §12): admitted to the phones' SIP
	// port and the audio range. linx-firewall-sync (FirewallSyncPlan) is
	// the only thing that ever adds or removes its elements; this file
	// only ever declares it, empty, with the rule that admits it.
	TrunkAddressSet = "trunk_addresses"
	// TrunkPlainAddressSet holds the addresses of trunks using a plain
	// transport (ADR-023): a subset of TrunkAddressSet, admitted to the
	// plain trunk port too.
	TrunkPlainAddressSet = "trunk_plain_addresses"
	// TrunkAudioForwardFile explains, in plain words, the one port forward
	// a home router sometimes needs for phone-line providers (docs/TRUNKS.md §8).
	TrunkAudioForwardFile = StackDir + "/TRUNK-AUDIO-FORWARD.txt"
)
...
func PhonesPlan(ctx context.Context, r Runner, lan LAN, fd FrontDoorSettings, readFile func(string) ([]byte, error)) (Plan, error) {
	...
	if lan.OK() {
		p = append(p, fileStep("Write the phone-line audio port forward steps", TrunkAudioForwardFile, trunkAudioForwardSteps(lan), 0o644, 0o755))
	}
	return p, nil
}

// trunkAudioForwardSteps explains the one port forward a home router
// sometimes needs (docs/TRUNKS.md §8): most providers answer audio to
// where Linx sent it from, so most homes need nothing here.
func trunkAudioForwardSteps(lan LAN) []byte {
	return fmt.Appendf(nil, `Generated by linx setup. Most homes need nothing here: read this only if a
test call to or from a phone-line provider (docs/TRUNKS.md) has audio one
way, or not at all.

Linx's own firewall already lets a phone-line provider's audio in, but only
from that provider's own address (kept up to date automatically as trunks
change, by linx-firewall-sync). Your router still has to send that traffic
to Linx in the first place:

1. On your router, forward UDP ports %d-%d to %s (this server).
2. Check it worked: sudo linx doctor ("Phone lines").

Nothing else needs forwarding: the SIP connection itself is one Linx makes
outward, or, for a provider that calls in, already opened for that
provider's address alone.
`, asteriskconf.RTPStart, asteriskconf.RTPEnd, lan.Address)
}
...
# Generated by linx setup (%[1]s). Changes are overwritten when setup runs again.
# Phones reach the phone system (SIP over TLS on %[2]d/tcp, encrypted audio on
# %[3]s/udp) from the local network only; unencrypted SIP (5060) from nobody.
# Phone-line providers that call in (docs/TRUNKS.md §12) reach the same SIP
# port and the audio range from their own address only, and the plain trunk
# port (%[9]d) only if that provider is confirmed unencrypted (ADR-023); this
# file only ever declares those addresses' sets empty — linx-firewall-sync
...
%[8]s	}
	...
	chain prerouting {
		...
		fib daddr type local tcp dport %[2]d ip saddr @%[10]s accept
		fib daddr type local udp dport %[3]s ip saddr @%[10]s accept
		fib daddr type local tcp dport %[9]d ip saddr @%[11]s accept
		fib daddr type local udp dport %[9]d ip saddr @%[11]s accept
		fib daddr type local tcp dport { 5060, %[2]d, %[9]d } counter drop
