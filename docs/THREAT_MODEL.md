...
| Phone engine image (Asterisk) | **T** tampered source or build | Built by Linx CI from the official release tarball: SHA-256 pinned, and its GPG signature checked against Sangoma's key with a pinned fingerprint; sound prompts pinned by SHA-256; base images pinned by digest; Trivy and cosign like every Linx image. The bundled pjproject is downloaded by Asterisk's build and checked against a checksum inside the signed tarball (see residual risks) | 1 |
| Phone engine container | **E** a flaw in Asterisk becomes a foothold | Only the modules Linx uses (no chan_sip, AGI, AMI, HTTP server); non-root (uid 100), read-only root filesystem, all capabilities dropped, `no-new-privileges`, memory and process limits; nothing in it listens except 5061 and the audio ports; its database login reads four views only; its ARI user is read-only | 1 |
...
| Media (1B) | **I** unencrypted audio | Every device requires SDES-SRTP (`media_encryption=sdes`, not optimistic): an offer without encryption is refused with 488 (tested). The SDES keys travel inside the TLS signalling. Asterisk stays in the audio path, so it decrypts and re-encrypts (a PBX-anchored call, as documented) | 1 |
...
- No registration lockout: repeated wrong passwords aren't blocked. Accepted while phones are LAN-only and passwords are 130 random bits; lockout is a precondition for public SIP (1C, docs/PBX.md §6).
