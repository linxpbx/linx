...
- Phone engine, step 4 (full STRIDE rows at step 6): the control plane now holds the `linx-services` provisioner password, so a compromised control plane can get 24 h certificates for any internal service name (not for public names, and not the CA's own). Accepted: it already holds the database and every other service secret. Asterisk proves itself to the control plane with `linx_ari_password` (HTTP Basic inside TLS), not a client certificate: Asterisk's websocket client could present one, but that would need a renewal loop inside the Asterisk container; revisit with Phase 2's device-certificate work. The ARI user Asterisk exposes over that websocket is read-only.
