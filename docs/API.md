...
**Alert sources** (each ships with the feature it watches): trunk down; WireGuard tunnel down (`wireguard.down:<profile-id>`, docs/TRUNKS.md §7); certificate renewal failure (`linx-certd` reports to the control plane); disk/storage nearly full (80 % warning, 90 % critical); backup failure; DDNS update failure; registration attack / toll-fraud detection; capacity limits; email sending broken; webhook endpoint disabled.

**As built (step 5)** — code: `internal/alert` (channel config, six senders, service, engine), `internal/store/alerts.go`, migration `0004_alerts.sql`.
- A channel's settings (URLs, tokens, chat id) are one JSON object, sealed with `dbsecret` like a webhook secret — never returned once saved; `POST .../test` is the only way to check a channel still works. A `webhook` channel's signing secret is generated by Linx, like a webhook endpoint's, not typed in by the admin.
- Reporting a problem (`Fire`) and clearing it (`Resolve`) are cheap, in-process calls a source makes as often as it likes (e.g. every health check) — they only record state. A background engine decides when that becomes a real notification: it waits `stable_since` + 5 minutes before the first send (the flap hold-back), fans out to every enabled channel whose `min_severity` the alert meets, and reminds every 24 h while still open.
- Quiet hours are per channel, in the channel's own local time (IANA zone); critical alerts bypass them unless the channel turns that off. Alerts that arrive during quiet hours are held (not dropped) and merged into one digest message once the channel's quiet hours end.
- Each notice is claimed before it's queued (a compare-and-set on the alert row), so an alert that clears between the engine's scan and its send, or a second control plane, never produces a stray or duplicate message. An alert that becomes more severe while open is re-sent on the next tick rather than at the next 24 h reminder.
- `alert.fired` and `alert.resolved` are also emitted as webhook events (docs/API.md §4), in the same transaction as the bookkeeping that marks the alert notified, so a subscriber sees exactly the alerts a human would.
- **First sources wired up**: webhook endpoint disabled (from step 4's worker, and from an admin turning one off or back on) and certificate renewal failure (the control plane polls `linx-certd`'s existing `/metrics` on `linx-private` every 5 minutes — not through the SSRF guard, since that's Linx's own service, not an admin-supplied URL; a warning once the deployed certificate has under 21 days left, critical under 7 — the thresholds `linx doctor` uses). **Disk/storage** and **DDNS update failure** are not wired up: the control plane has no filesystem of its own to check (no volumes; `read_only: true`) and no DDNS-updater feature exists yet to watch — see `docs/THREAT_MODEL.md`. Backup failure, registration attack, capacity limits and email sending broken ship with those features.
...
5. **Admin alerts**: engine, six channels, first sources (certificate renewal, disk, DDNS, webhook disabled). *(Sonnet)* **Done 2026-09-23** — disk and DDNS deferred (§5 "As built"); the rest as specified.
6. **Review + docs**: security review, threat model, `linx doctor` checks, `docs/DEMO_PHASE1A.md`. *(Opus)* **Done 2026-09-24** — findings and fixes in `docs/THREAT_MODEL.md` "Phase 1A review".
