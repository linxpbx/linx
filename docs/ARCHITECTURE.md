...
| `linx-coturn` | STUN/TURN on UDP 443 and TLS 443, with time-limited REST credentials | coturn |
...
Clients pin **root** SPKIs (ISRG Root X1/X2 plus the ZeroSSL roots), never leaf or intermediate certificates. Pin-set updates are pushed over the authenticated config channel before any CA change. Reload methods for each component are documented in `docs/ops/CERT_RELOAD.md` (Phase 0b).
