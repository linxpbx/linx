...
- **Linphone**, free, on your iPhone (App Store) and a second device: your Mac (linphone.org) or an iPad. Wherever this list says "Mac", use the second device. It's only a test phone; Linx never ships it.
...
## 1. Docs
- [x] `docs/PBX.md`, ADR-031 to ADR-035 in `docs/DECISIONS.md` and the "Phone engine" rows and "Phase 1B review" in `docs/THREAT_MODEL.md` read sensibly to you.
...
- [x] Setup says "Phones will be able to connect from your local network (192.168.1.0/24), where this server is 192.168.1.50" (your numbers), and says it's writing the firewall rules and restarting Docker once.
- [x] It ends with "Linx is running", "Trusted certificate issued for *.lab.linxpbx.com", and a **Phones** block asking for a DNS record.
...
```
- [x] `linx-postgres`, `linx-control-plane` and `linx-asterisk` are `healthy`; `linx-step-ca` healthy, `linx-certd` running.
...
```
- [x] The key lists `devices:write` among its scopes; the webhook is created.
...
- [x] **Messages:** dial `555` (no such number): "the number you have dialled is not in service"-style message. Dial `101` from the iPhone itself: "nobody is available" message.
- [x] **A real call:** from the iPhone dial `102`. The Mac rings, shows "iPhone" / `101` as the caller; answer; both sides hear each other clearly for about 20 seconds; hang up.
- [x] While it's up: `api $API/calls/active | jq` shows one call, `state` `answered`, from `101` to `102`, and `"phone_engine_connected": true`.
- [x] On webhook.site: `call.started`, `call.answered` (with `answered_by`) and `call.ended` (`outcome` `answered`, `duration_seconds` about 20), all with the same `id`. **This is the core of the Phase 1B exit.**
- [x] **Nobody answers:** call `102` again and don't answer. After 30 s the iPhone hears "nobody is available"; webhook.site gets `call.missed`.
- [x] **Unencrypted audio is refused:** on the Mac, set Media encryption to **None** and call `*43`. The call fails at once (Linphone shows an error, e.g. "Not acceptable"). Set it back to SRTP (mandatory).
...
```
- [x] The first answers `204`; webhook.site gets `device.revoked`. Trying to turn it back on answers `"device_revoked"`: a revoked device stays revoked (add a new device instead).
- [x] From the Mac, call `101`: straight to "nobody is available" (the iPhone doesn't ring).
- [x] From the iPhone, call `102` or `*43`: the call fails. Within a minute, or when you reopen the app, its account shows an error. **This is the Phase 1B exit.**
...
```
- [x] 5061 listens on the LAN address only, and nothing on 5060. The firewall table lists your network.
- [x] The new secrets `linx_asterisk_db_password`, `linx_ari_password` and `linx_ca_services_password` are there with the others.
- [x] Asterisk and the API service don't run as root.
- [x] `f|f`: Asterisk's database login can't read API keys, or even the device table (only its views).
- [x] The last count is `0`: the Mac's password isn't in the audit log or webhook payloads.
...
```
- [x] `TLSv1.3` and `Verify return code: 0 (ok)`.
...
```
- [x] Doctor reports a `problem` under Phone system (not running) with a `Fix:` line, and exit code 1. The Mac's account goes red. After `up`, within a minute doctor is all green again, the Mac reconnects on its own (or when you reopen it), and `*43` works again.
...
- 2026-09-24, Ubuntu 24.04 VM (the Phase 0/1A server, cleaned; bridged, 192.168.1.212), `lab.linxpbx.com` via Cloudflare with a trusted certificate, iPhone + iPad with Linphone 6.2: **passed** on commit `ae653f1`, after these fixes found during the run (each with a new automated test): Asterisk crash-looped after any restart (tmpfs ownership, `ba4502b`); Linphone offers Opus first, so Linx's spoken messages were silent and the call was cut (G.722 first, `6dc6d68`); Asterisk named itself by its container address, so calling back from Linphone's history failed (`from_domain`, `ae653f1`); setup didn't offer to keep the DNS token after the domain was corrected (`b1d403f`). Operator notes, not defects: the first setup used `sip.lab.linxpbx.com` as the domain (setup adds `sip.` itself); the `sip.lab` DNS record first pointed at the home's public address; the iPad's Linphone had encryption "if possible" rather than mandatory, so its calls were refused (488) until changed. Still open: a renewed certificate only reaches Asterisk on restart (`docs/ops/CERT_RELOAD.md`), and doctor waits while Asterisk is starting instead of saying so; both are the next task.
