...
- **Endpoint settings Linx sets for every device:** TLS transport only, `media_encryption=sdes` (becomes `dtls` for web/iOS devices later), encryption required, codecs `opus,g722,ulaw`, direct media off (Asterisk stays in the audio path, ARCHITECTURE §4.1), `max_contacts=1`, limit of 2 calls at once, ACL = LAN in this slice.
