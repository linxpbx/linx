...
| Modules | Only what Linx uses (`menuselect`): PJSIP, SRTP, ODBC realtime, ARI, dialplan basics, `app_echo`, `func_odbc`, G.722/G.711, Opus with transcoding (Wazo's open-source codec, Phase 1C, ADR-041), the secure websocket transport for browsers (Phase 1C). No chan_sip, AGI, AMI-over-network, telephony cards or add-ons. |
...
| Networks | `linx-private` (Postgres, control plane ↔ ARI), `linx-public`, and (Phase 1C) `linx-sipws`: internal, shared with the control plane only, where Asterisk's browser websocket listens (docs/WEB.md §2). |
...
| Certificates | The public certificate from `linx-certd` (`certs` volume, read-only), so phones trust it without extra setup. (Phase 1C: the browser websocket's own internal certificate comes from the control plane; docs/WEB.md §2.) Reload on renewal: the entrypoint watches certd's `current` symlink and runs `module reload res_pjsip.so` within a minute; no calls dropped (`docs/ops/CERT_RELOAD.md`). The entrypoint therefore stays as Asterisk's parent (forwarding `docker stop`'s signal), with compose's `init: true` as PID 1. |
...
- **Endpoint settings Linx sets for every device:** TLS transport only, `media_encryption=sdes`, encryption required, codecs `opus,g722,ulaw` in the caller's own order (migration 0011, Phase 1C: Opus first again now that the image can encode it, ADR-041; migration 0009 had put G.722 first with `prefer:configured` because a phone that put Opus first heard no messages). **Web devices** (kind `web`, migration 0011; created only by the web client's sign-in, from Phase 1C step 4) instead use the secure websocket (`transport-wss`) and WebRTC media: ICE, `media_encryption=dtls` with fingerprint checking, `dtls_setup=actpass`, an auto-generated DTLS certificate, AVPF and rtcp-mux (docs/WEB.md §2). direct media off (Asterisk stays in the audio path, ARCHITECTURE §4.1), `from_domain` = `sip.<domain>` (migration 0010, from the control plane's `LINX_DOMAIN` at every start; without it phones saw Asterisk's container address and Linphone's call history dialled it), `max_contacts=1`, limit of 2 calls at once, ACL = LAN in this slice.
...
- **1D:** trunks (TLS/SRTP first, ADR-023 fallback, WireGuard ADR-024), outbound rules, toll-fraud limits, Opus↔G.711 transcoding at the trunk. Designed in `docs/TRUNKS.md`.
