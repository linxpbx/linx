...
## Current state
- Phase 0a approved 2026-09-23. Phase 0b items 1 (skeleton), 2 (CI), 3 (installer prereqs), 4a (linx-certd: Cloudflare + DuckDNS, wildcard default) and 4b (step-ca bootstrap in `linx setup`; `make test-docker`) done. Next: setup wiring for domain/DNS token and a live staging test on a linxpbx.com subdomain (Cloudflare).
